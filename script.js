...
function showHome(){document.body.classList.remove("quote-active");bayiYetkili=false;$("quoteScreen").classList.add("hidden");$("homeScreen").classList.remove("hidden");$("savedBox").classList.add("hidden");$("cutBox").classList.add("hidden");$("passwordBox").classList.add("hidden");$("musteriSifreBox").classList.add("hidden")}
function showQuote(tip){document.body.classList.add("quote-active");girisTipi=tip;$("homeScreen").classList.add("hidden");$("quoteScreen").classList.remove("hidden");$("screenTitle").innerText=tip==="bayi"?"Bayi Teklif Ekranı":"Müşteri Teklif Ekranı";$("mod").value=tip==="bayi"?"bayi":"son";$("mod").disabled=tip==="musteri";$("onOdemeWrap").style.display="block";clearQuote()}
...
function showSavedFromQuote(){document.body.classList.remove("quote-active");
  if(!sonTeklifSifreKontrol()) return;
...
function clearQuote(){$("rows").innerHTML="";$("musteri").value="";$("telefon").value="";$("adres").value="";$("onOdeme").value="";$("tarih").value=bugun();for(let i=0;i<10;i++)addRow();calc()}
...
function loadQuote(id){document.body.classList.add("quote-active");const q=getSaved().find(x=>x.id===id);if(!q)return;const tip=q.type||"bayi";if(tip==="bayi"&&!bayiYetkili){const sifre=prompt("Bayi teklifini açmak için şifre girin:");if(sifre!=="4321"){alert("Şifre hatalı");return}bayiYetkili=true}girisTipi=tip;$("homeScreen").classList.add("hidden");$("quoteScreen").classList.remove("hidden");$("savedBox").classList.add("hidden");$("cutBox").classList.add("hidden");$("screenTitle").innerText=girisTipi==="bayi"?"Bayi Teklif Ekranı":"Müşteri Teklif Ekranı";$("mod").value=q.mod||(girisTipi==="musteri"?"son":"bayi");$("mod").disabled=girisTipi==="musteri";$("onOdemeWrap").style.display="block";$("tarih").value=q.date||bugun();$("musteri").value=q.customer||"";$("telefon").value=q.phone||"";$("adres").value=q.address||"";$("onOdeme").value=q.totalPaid??0;$("fiyatGorunum").value=q.fiyatGorunum||"satir";$("rows").innerHTML="";(q.rows||[]).forEach(r=>addRow(r));while(document.querySelectorAll(".line").length<10)addRow();calc()}
